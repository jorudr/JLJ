    ...
    <div class="min-h-0 flex-1 overflow-y-auto pr-4 pb-10 custom-scrollbar">
      <div v-if="activeProtocolTab === 'PYRAMIDING' || activeProtocolTab === 'AVERAGING_DOWN'" class="flex flex-col gap-4 transition-all">
        <div v-for="(ent, idx) in activeMultipleEntries" :key="ent.id" class="flex items-center gap-4">
          ...
          <div class="flex flex-1 flex-col gap-1">
            <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'УРОВЕНЬ_ЦЕНЫ' : 'Price_Lvl' }}</span>
          ...
          <div class="flex flex-1 flex-col gap-1">
            <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'РАЗМЕР_ЛОТА' : 'Lot_Size' }}</span>
        ...
        <div v-if="showAutoPrompt" class="mt-2 flex flex-col gap-3 border border-black/30 bg-black/5 p-3 dark:border-white/30 dark:bg-white/5">
          <div class="flex items-center gap-3">
            <div class="flex-1">
              <span class="mb-1 block text-[7px] font-bold uppercase tracking-[0.4em] opacity-60">{{ locale === 'ru' ? 'Базовая_Цена' : 'Base_Price' }}</span>
            ...
            <div class="flex-1">
              <span class="mb-1 block text-[7px] font-bold uppercase tracking-[0.4em] opacity-60">{{ locale === 'ru' ? 'РАЗМЕР_ЛОТА' : 'Lot_Size' }}</span>
      ...
      <div v-if="activeProtocolTab === 'EXIT'" class="flex flex-col gap-4 transition-all">
        <div v-for="(ent, idx) in exitEntries" :key="ent.id" class="flex items-center gap-4">
          <span class="w-6 text-[8px] font-mono font-black tracking-widest opacity-40">#{{ idx + 1 }}</span>
          <div class="flex flex-1 flex-col gap-1">
            <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'УРОВЕНЬ_ВЫХОДА' : 'Exit_Lvl' }}</span>
            <input v-model="ent.price" type="text" inputmode="decimal" placeholder="0.00" class="nier-input w-full border-b border-black/20 pb-1 !text-black dark:border-white/20 dark:!text-white" @input="sanitizeInlineNumberInput($event, ent, 'price')" />
          </div>
          <div class="flex flex-1 flex-col gap-1">
            <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'РАЗМЕР_ЛОТА' : 'Lot_Size' }}</span>
    ...
    <div class="shrink-0 border-t nier-border-primary pt-6">
      <div v-if="activeProtocolTab === 'PYRAMIDING' || activeProtocolTab === 'AVERAGING_DOWN'" class="flex items-center justify-between" :class="{ 'opacity-30 grayscale': !entryMethodEnabled }">
        <div class="flex flex-col gap-1">
          <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'СРЕДНЯЯ ЦЕНА ВХОДА' : 'Aggregated_Avg_Entry' }}</span>
          <span class="text-sm font-mono font-black">{{ averageEntry > 0 ? averageEntry.toFixed(5) : '0.00' }}</span>
        </div>
        <div class="flex flex-col items-end gap-1">
          <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'ОБЩИЙ ОБЪЕМ' : 'Total_Volume' }}</span>
      ...
      <div v-if="activeProtocolTab === 'EXIT'" class="flex items-center justify-between">
        <div class="flex flex-col gap-1">
          <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'СРЕДНЯЯ ЦЕНА ВЫХОДА' : 'Aggregated_Avg_Exit' }}</span>
          <span class="text-sm font-mono font-black">{{ averageExit > 0 ? averageExit.toFixed(5) : '0.00' }}</span>
        </div>
        <div class="flex flex-col items-end gap-1">
          <span class="text-[7px] font-bold uppercase tracking-[0.4em] opacity-40">{{ locale === 'ru' ? 'ОБЩИЙ ОБЪЕМ ВЫХОДА' : 'Total_Exit_Volume' }}</span>
