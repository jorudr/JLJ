  ...
  <div class="w-full flex flex-col pt-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div class="flex flex-col items-end gap-4">
        ...
        <div class="flex items-center gap-1 bg-black/5 dark:bg-white/5 p-1 rounded-xl w-fit">
          <button 
            ...
            :class="historyStore.timeFilter === f.id ? 'bg-white dark:bg-black shadow-sm text-black dark:text-white' : 'opacity-50 hover: dark:hover:'"
          >
            {{ f.label }}
          </button>
        </div>
        ...
        <div v-if="historyStore.availableCategories.length > 0" class="flex flex-wrap justify-end gap-2 max-w-md">
            <button 
              ...
              :class="historyStore.categoryFilter === 'all' ? 'bg-black text-white border-black dark:bg-white dark:text-black dark:border-white' : 'border-black/10 dark:border-white/10 opacity-40 hover:text-black dark:hover:text-white'"
            >
              All Categories
            </button>
            <button 
              ...
              class="px-3 py-1.5 text-[9px] uppercase tracking-wider font-bold rounded-full border transition-all duration-300"
              :class="historyStore.categoryFilter === cat ? 'bg-black text-white border-black dark:bg-white dark:text-black dark:border-white' : 'border-black/10 dark:border-white/10 opacity-40 hover:text-black dark:hover:text-white'"
            >
              {{ capitalize(cat) }}
            </button>
        </div>
    ...
    <div v-if="historyStore.groupedHistory.length > 0" class="space-y-16">
      <div v-for="[groupName, items] in historyStore.groupedHistory" :key="groupName" class="px-4">
        <h4 class="text-[9px] uppercase tracking-[0.3em] font-medium opacity-40 mb-8 flex items-center gap-4">
          {{ groupName }}
              ...
              <div class="flex items-center gap-4 mb-2">
                ...
                <div class="flex items-center gap-3 text-[10px] opacity-40 font-serif italic">
                  <div class="flex items-center gap-1.5">
                    <span class="font-medium opacity-80">by {{ item.authorName || 'Anonymous' }}</span>
                  </div>
                  <span>·</span>
    ...
    <div v-else class="flex flex-col items-center justify-center py-32 text-center">
      ...
      <button 
        ...
        class="mt-4 text-[9px] uppercase tracking-widest font-bold opacity-60 hover:opacity-70 transition"
      >
        Reset Filters
      </button>
    </div>
    ...
    <div v-if="historyStore.history.length > 0" class="mt-24 px-4 flex justify-start">
        <button 
          ...
          class="text-[9px] uppercase tracking-widest font-bold opacity-50 hover:opacity-100 transition-colors flex items-center gap-2"
        >
    ...
    <transition name="fade">
      <div v-if="isConfirmingClear" class="fixed inset-0 z-[1000] flex items-center justify-center p-6">
        ...
        <div class="relative bg-white dark:bg-black rounded-2xl p-8 max-w-sm w-full border border-black/5 dark:border-white/10 shadow-2xl text-center">
          <h4 class="text-xl font-serif mb-2">Clear History?</h4>
          <p class="text-xs opacity-50 mb-8 italic leading-relaxed">This will permanently remove your local archive of viewed threads.</p>
          <div class="flex gap-3">
            ...
            <button @click="clearHistory" class="flex-1 py-3 text-[10px] uppercase tracking-widest font-bold rounded-lg bg-current text-white dark:text-black hover:opacity-90 transition">Purge</button>
          </div>
        </div>
      </div>
    </transition>
