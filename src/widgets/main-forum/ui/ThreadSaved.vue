    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 px-4">
      ...
      <div v-if="savedThreads.availableCategories.length > 0" class="flex flex-wrap justify-end gap-2 max-w-md">
          <button 
            ...
            :class="savedThreads.categoryFilter === 'all' ? 'bg-black text-white border-black dark:bg-white dark:text-black dark:border-white' : 'border-black/10 dark:border-white/10 text-gray-400 hover:text-black dark:hover:text-white'"
          >
            All Categories
          </button>
          <button 
            ...
            class="px-3 py-1.5 text-[9px] uppercase tracking-wider font-bold rounded-full border transition-all duration-300"
            :class="savedThreads.categoryFilter === cat ? 'bg-black text-white border-black dark:bg-white dark:text-black dark:border-white' : 'border-black/10 dark:border-white/10 text-gray-400 hover:text-black dark:hover:text-white'"
          >
            {{ capitalize(cat) }}
          </button>
      </div>
    ...
    <div v-if="savedThreads.filteredSavedThreads.length > 0" class="grid gap-4 px-4">
      ...
      >
        <div>
          <div class="flex items-center gap-4 mb-2">
            ...
            <div class="flex items-center gap-3 text-[10px] text-gray-400 font-serif italic">
              <span class="font-medium text-[#c49b6b] dark:text-[#d8b488]">by {{ getAuthorName(thread.authorId) }}</span>
              <span>·</span>
        ...
        <div class="flex items-center gap-6">
            ...
            >
                <svg class="w-4 h-4 fill-emerald-500 stroke-emerald-500" viewBox="0 0 24 24">
