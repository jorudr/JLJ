<template>
  <div class="home-details" :lang="locale">
    ...
    <section class="home-block" aria-labelledby="plans-title">
      ...
      <div class="plans">
        <article v-for="plan in ['free', 'full']" :key="plan" class="plan" :class="{ 'plan-full': plan === 'full' }">
          <h3>{{ t('landing.pricing.' + plan + '.name') }}</h3>
          <p class="price">{{ t('landing.pricing.' + plan + '.price') }} <span>{{ t('landing.pricing.' + plan + '.period') }}</span></p>
          <ul>
            <li v-for="feature in tm('landing.pricing.' + plan + '.features')" :key="feature.title">{{ feature.title }}</li>
          </ul>
          <router-link :to="plan === 'free' ? '/download' : '/pricing'" class="action">{{ plan === 'free' ? copy.download : t('landing.pricing.full.cta') }} <span aria-hidden="true">↗</span></router-link>
        </article>
      </div>
...
.plan { display: flex; flex-direction: column; padding: clamp(24px, 3vw, 40px); border: 1px solid #ffffff30; }
.plan-full { background: #f5f5f0; color: #000; }
.price { margin-top: 14px; font: 2.8rem 'Cormorant Garamond', serif; color: inherit; }
.price span { font: .8rem ui-monospace, monospace; opacity: .65; }
.plan ul { margin: 28px 0; padding-left: 18px; font: .9rem/1.7 ui-sans-serif, system-ui, sans-serif; }
...
.plan-full .action { background: #000; color: #f5f5f0; border-color: #000; }
