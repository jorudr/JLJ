...
Когда сезон завершён (`season.endsAt <= now` или `season.status: "closed"`), Worker один раз выдаёт награды первым трём участникам leaderboard. Если очки одинаковые, участники получают одинаковый prize-rank: например два первых места с одинаковыми очками получают `prize-1`. Если `prize-3` отсутствует, третий участник получает `prize-2`.
